...
public class LuceneManager implements LuceneService {
  ...
  public void save(List<Article> articles) {
    ...
    for (Article article : articles) {
      ...
      if (Objects.nonNull(article.getCreateBy())) {
        document.add(
            new StringField("createBy", String.valueOf(article.getCreateBy()), Field.Store.YES));
      }
      if (Objects.nonNull(article.getUpdateBy())) {
        document.add(
            new StringField("updateBy", String.valueOf(article.getUpdateBy()), Field.Store.YES));
      }
  ...
  public List<Article> search(LuQueryParam queryParam) {
    ...
    for (int i = 0; i < topDocs.scoreDocs.length; i++) {
      ...
      try {
        ...
        article = new Article();
        String hlTitle =
            highlighter.getBestFragment(analyzer, "title", doc.getField("title").stringValue());
        ...
        String hlSummary =
            highlighter.getBestFragment(analyzer, "summary", doc.getField("summary").stringValue());
  ...
  public Document queryByDocId(int docId) {
    ...
    try {
      IndexSearcher indexSearcher =
          new IndexSearcher(DirectoryReader.open(FSDirectory.open(Path.of(indexFileDir))));
  ...
  public void deleteByIds(List<String> ids) {
    ids = ids.stream().filter(Strings::isNotBlank).collect(Collectors.toList());
    ...
    String queryStr = stringJoiner.toString();
    try {
  ...
  public void deleteByIds(List<String> ids, IndexWriter indexWriter) {
    if (CollectionUtils.isEmpty(ids)) {
      return;
    }
